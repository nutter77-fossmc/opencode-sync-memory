...
import { isRepo, initRepo, hasRemote, setRemote, commitAll, rebasePull, push, resolveConflict, continueRebase, cloneRepo, getRemoteUrl, } from "./git";
...
export const SyncMemoryPlugin = async (ctx) => {
    const $ = ctx.$;
    ...
    async function sync() {
        ...
        if (!repoOk)
            return "Memory store not initialized. Save a memory first.";
        const remoteUrl = config.remote.url || (await hasRemote($) ? await getRemoteUrl($) : "");
        if (!remoteUrl)
            return "No remote configured. Set it in ~/.config/opencode/opencode-sync-memory.jsonc";
        if (!(await hasRemote($))) {
    ...
    return {
        tool: {
            memory_save: tool({
                description: "Save a fact, decision, preference, or any knowledge to persistent memory. " +
                ...
                args: {
                    ...
                    category: tool.schema
                        ...
                        .optional()
                        .default("notes")
                        .describe("Category: preferences, repos, technical, people, workflows, snippets, or notes"),
                    tags: tool.schema
                        .string()
                        .optional()
                ...
                async execute(args, _context) {
                    ...
                    const path = await saveMemory(args.category || "notes", args.title, args.content, {
                        tags: args.tags,
            ...
            memory_search: tool({
                description: "Search memories, daily notes, and optionally past opencode sessions. " +
            ...
            memory_read: tool({
                description: "Read the full content of a specific memory by its path. " +
                ...
                async execute(args) {
                    ...
                    if (!content)
                        return `Memory not found: ${args.path}`;
                    return `# ${content.fm.title || args.path}\n\n${content.body}`;
                },
            }),
            memory_list: tool({
                description: "List memories, optionally filtered by category. Shows title, category, project, and last updated.",
                args: {
                    category: tool.schema
                        .string()
                        .optional()
            ...
            memory_update: tool({
                description: "Update an existing memory's content, title, tags, or importance.",
                args: {
                    path: tool.schema
                        .string()
            ...
            memory_forget: tool({
                description: "Remove a memory from the active store. It's archived to .archive/ for safety.",
                args: {
                    path: tool.schema
                        .string()
                ...
                async execute(args, _context) {
                    ...
                    if (args.reason) {
                        await appendToDailyNote($, "Memory Removal", `Removed ${args.path}: ${args.reason}`);
                    }
            ...
            memory_status: tool({
                description: "Show the current status of the memory store: size, category counts, sync status, and remote info.",
                args: {},
                async execute() {
                    const repoOk = await isRepo($);
