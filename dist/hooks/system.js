...
export async function getSystemContext($, project) {
    const config = await loadConfig();
    if (!config.injection.enabled)
        return "";
    const ctx = await buildActiveContext($, project, config.injection.maxMemories, config.injection.maxLinesPerMemory);
